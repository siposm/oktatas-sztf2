...
namespace program
{
    class BinarisKeresoFa_genKulcs < T , K > where K : IComparable
    {
        ...
        public BinarisKeresoFa_genKulcs()
        {
        ...
        private void Beszuras(ref FaElem p, K kulcs, T tartalom)
        {
            if (p == null)

                p = new FaElem(kulcs, tartalom);

            else if (p.kulcs.CompareTo(kulcs) < 0) // p.kulcs < kulcs (p.kulcs-hoz viszonyítva a kulcs-ot, az kisebb)

                Beszuras(ref p.jobb, kulcs, tartalom);

            else if (p.kulcs.CompareTo(kulcs) > 0) // p.kulcs > kulcs
